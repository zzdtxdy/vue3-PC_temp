 ...
 * @LastEditors: zhongzd
 * @LastEditTime: 2025-01-20 17:22:46
...
const activeTopMenuPath =
  useRoute().path.split('/').filter(Boolean).length > 1 ? useRoute().path.match(/^\/[^/]+/)?.[0] || '/' : '/'
...
const navigateToFirstLeftMenu = (menus: RouteRecordRaw[]) => {
  ...
  if (firstMenu.children && firstMenu.children.length > 0) {
    ...
    router.push({
      name: firstMenu.name,
      query: typeof firstMenu.meta?.params === 'object' ? (firstMenu.meta.params as LocationQueryRaw) : undefined
    })
  }
}
