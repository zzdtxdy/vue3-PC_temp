  ...
  <div class="wh-full" :class="classObj">
    ...
    <div v-if="layout === LayoutEnum.MIX" class="mix-container">
      <div class="mix-container-sidebar">
        <el-scrollbar>
          <SidebarMenu :menu-list="mixedLayoutLeftRoutes" :base-path="activeTopMenuPath" />
        </el-scrollbar>
        <div class="sidebar-toggle">
          <hamburger :is-active="globalStore.sidebar.opened" @toggle-click="toggleSidebar" />
        </div>
      </div>
...
const isOpenSidebar = computed(() => globalStore.sidebar.opened)
const showTagsView = computed(() => globalStore.tagsView) // 是否显示tagsView
const layout = computed(() => globalStore.layout) // 布局模式 left top mix
const activeTopMenuPath = computed(() => globalStore.activeTopMenuPath) // 顶部菜单激活path
const mixedLayoutLeftRoutes = computed(() => authStore.mixedLayoutLeftRoutes) // 混合布局左侧菜单

watch(
  () => activeTopMenuPath.value,
  (newVal: string) => {
    authStore.setMixedLayoutLeftRoutes(newVal)
  },
  {
    deep: true,
    immediate: true
...
const classObj = computed(() => ({
  hideSidebar: !globalStore.sidebar.opened,
  openSidebar: globalStore.sidebar.opened,
  mobile: globalStore.device === DeviceEnum.MOBILE,
  [`layout-${globalStore.layout}`]: true
}))

watchEffect(() => {
  globalStore.toggleDevice(width.value < WIDTH_DESKTOP ? DeviceEnum.MOBILE : DeviceEnum.DESKTOP)
  if (width.value >= WIDTH_DESKTOP) {
    globalStore.openSideBar()
  } else {
